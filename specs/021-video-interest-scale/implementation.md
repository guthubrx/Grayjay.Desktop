...
- **Build d'integration**: le build complet `osx-arm64` a ensuite reussi dans la worktree d'integration isolee au commit `33d709e`, avec les sous-modules requis. Les assets de `Grayjay.Desktop.Web/dist` ont ete compares sans difference avec ceux installes dans `/Applications/BlueJay.app/Contents/Resources/wwwroot/web`.
- **Livraison locale**: `/Applications/BlueJay.app` a ete sauvegardee dans `/Applications/BlueJay.app.backup-20260807-121325`, remplacee par le bundle issu de `33d709e`, puis relancee. La validation visuelle de la note issue d'un profil editorial et du backfill reste a effectuer dans l'application.
