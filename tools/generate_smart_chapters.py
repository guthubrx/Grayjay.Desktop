...
def build_prompt(task: VideoTask, cues: list[TranscriptCue], args: argparse.Namespace, analysis: dict[str, Any]) -> str:
    ...
    - SPREAD the scores across the FULL range and score RELATIVELY within THIS video, not in absolute terms. Compare the sections to each other: the single best moment(s) MUST reach >= 0.93, the weakest substantive parts MUST go down to about 0.50-0.60, and genuine filler below 0.50. Do NOT cluster everything in 0.70-0.85 — that defeats the purpose. Aim for real contrast: a few clear peaks (>= 0.90), a spread of middle values, and visible low points. If two sections differ in value, their scores MUST differ.
