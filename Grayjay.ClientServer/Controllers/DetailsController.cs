...
namespace Grayjay.ClientServer.Controllers
{
    ...
    public class DetailsController : ControllerBase
    {
        ...
        public async Task<VideoLoadResult> VideoLoad(string url)
        {
            ...
            prepared?.Dispose();
            Logger.i(nameof(DetailsController), $"playback_prefetch status={consumeStatus.ToString().ToLowerInvariant()} sourceReady={resolved.SourceReady} manifestReady={resolved.ManifestReady} url=[{url}]");
            return resolved;
        }
        ...
        public async Task<VideoPrepareResult> VideoPrepare(string url)
        {
            ...
            try
            {
                ...
                Logger.i(nameof(DetailsController), $"playback_prefetch status={status} sourceReady={result.Value?.SourceReady == true} manifestReady={result.Value?.ManifestReady == true} elapsedMs={watch.ElapsedMilliseconds} url=[{url}]");
                return new VideoPrepareResult()
                {
                    Url = url,
                    Status = status,
