...
function httpGETBypass(url, headers, ct) {
    return JSON.parse(syncPOST("/Developer/get?CT=" + ct, {"Content-Type": "application/json"}, JSON.stringify({
        url: url,
        headers: headers
    })));
}
...
function pluginCaptchaTestPlugin(url, html) {
    return syncPOST("/Developer/captchaTestPlugin?url=" + url, {}, html);
}
function pluginLogoutTestPlugin() {
    return syncGET("/Developer/logoutTestPlugin", {});
}
function pluginGetPackage(packageName) {
    return JSON.parse(syncGET("/Developer/packageGet?variable=" + packageName, {}));
}
function pluginRemoteProp(objID, propName) {
    return JSON.parse(syncGET("/Developer/remoteProp?id=" + objID + "&prop=" + propName, {}));
}
function pluginRemoteCall(objID, methodName, args) {
    return JSON.parse(syncPOST("/Developer/remoteCall?id=" + objID + "&method=" + methodName, { "Content-Type": "application/json" }, JSON.stringify(args)));
}
function pluginRemoteTest(methodName, args) {
    return JSON.parse(syncPOST("/Developer/remoteTest?method=" + methodName, { "Content-Type": "application/json" }, JSON.stringify(args)));
}

function pluginIsLoggedIn(cb, err) {
    fetch("/Developer/isLoggedIn", {
        timeout: 1000
    })
        .then(x => x.json())
        .then(x => cb(x))
...
function uploadDevPlugin(config) {
    return JSON.parse(syncPOST("/Developer/loadDevPlugin", { "Content-Type": "application/json" }, JSON.stringify(config)));
}
function getDevLogs(lastIndex, cb) {
    if(!lastIndex)
        lastIndex = 0;
    fetch("/Developer/getDevLogs?index=" + lastIndex, {
        timeout: 1000
    })
        .then(x=>x.json())
        .then(y=> cb && cb(y));
...
function getDevHttpExchanges(cb) {
    fetch("/Developer/getDevHttpExchanges", {
        timeout: 1000
    })
        .then(x=>x.json())
        .then(y=> cb && cb(y));
...
function sendFakeDevLog(devId, msg) {
    return syncGET("/Developer/fakeDevLog?devId=" + devId + "&msg=" + msg, {});
}
